...
int main (int argc, char* argv[]) {
    ...
    const std::vector<std::string> cmdArgs = { argv, argv + argc };  // store arguments, options and flags from the command line
    ...
    generateFm( emudata, emusize, req_chunksize, pattern_id, idle_chars, random_sz, crc_new, use_streamid, add_busy, omit_one_soc, omit_one_eoc, add_crc_err );



    for (int i = 0; i<emusize; i++) {
	output << "FE_EMU_CONFIG_WRADDR=0x"<<hex<<i<< endl;
	output << "FE_EMU_CONFIG_WRDATA=0x"<<hex << emudata[i] << endl;
	output << "FE_EMU_CONFIG_WE=1" << endl;
	output << "FE_EMU_CONFIG_WE=0" << endl;
    }
    output.close();
}
