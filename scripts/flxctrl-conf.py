...
from daqconf.detreadoutmap import DetReadoutMapService
...
def cli(config, detector_readout_map_file, emulator_mode, json_dir, debug):
    ...
    detro_map = DetReadoutMapService()
    detro_map.load(flxconf.detector_readout_map_file)

    ru_descrs = dro_map.get_ru_descriptors()
    ...
    for ru_name, ru_desc in ru_descrs.items():
        ...
        nickname = 'flxcard_' + ru_desc.host_name + '_card_' + str(ru_desc.iface)
        ...
        if boot.use_k8s:
            the_system.apps[nickname].resources = {
                f"felix.cern/flx{ru_desc.iface*2}-ctrl": "1", # requesting FLX {ru_desc.iface*2}
            }
